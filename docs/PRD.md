...
| **문서 상태** | Draft (검토 및 주요 결정 사항 답변 대기 중) |
...
## 29. 확인 필요 사항 (Open Questions)

다음 항목은 구현 진행 전 **사용자(PM/기획자)의 최종 결정이 필요한 주요 제품 파라미터**입니다. 각 질문에 대해 권장안과 사유를 함께 제시합니다.

---

### ❓ 질문 1: 고양이 상태 지수(5종)의 실시간 자동 감소(Tick) 주기 및 감소폭 설정
* **내용**: 사용자가 조작하지 않을 때 배고픔, 에너지, 스트레스 등의 수치가 자동으로 변화하는 주기(Tick)를 얼마로 설정할 것인가?
* **옵션**:
  * **A안 (권장)**: **30초 단위 Tick** (배고픔 +3, 에너지 -2). 대화형 프로토타입 시연에 적합하며 빠른 피드백 확인 가능.
  * B안: 5분 단위 Tick. 실제 타마고치 스타일의 리얼타임 육성 게임에 적합.
* **권장 이유**: 캠프 시연 및 프롬프트 프로토타이핑 검증 환경에서는 30초~1분 단위의 다소 빠른 수치 변동이 있어야 센서 연동 및 상태 변화를 즉시 시각적으로 확인하기 용이합니다.

---

### ❓ 질문 2: Arduino 센서 이벤트 디바운스(Debounce) 및 스팸 방지 시간 기준
* **내용**: 터치 센서나 버튼이 짧은 시간에 여러 번 다중 입력될 때 스팸으로 처리할 디바운스 시간의 기준은 얼마가 적절한가?
* **옵션**:
  * **A안 (권장)**: **300ms 디바운스** 적용 + 동일 입력 3초 내 4회 이상 시 '스트레스 증가'로 게임 로직에 반영.
  * B안: 1000ms(1초) 무조건 차단 (연속 입력 완전 무시).
* **권장 이유**: 사용자의 연타 행동 자체를 게임 내 '고양이를 귀찮게 하는 행동(스트레스 요인)'으로 반영하기 위해, 센서 오류성 바운싱만 300ms로 잡고 연속 입력은 고양이 기분 변화로 전환하는 A안이 훨씬 풍부한 인터랙션을 제공합니다.

---

### ❓ 질문 3: 포토 카드 캡처 시 오버레이 메타데이터 디자인 스타일 선택
* **내용**: 방 캡처 이미지 하단에 삽입될 고양이 이름, 날짜, 상태 문구의 오버레이 디자인 방식을 어떻게 처리할 것인가?
* **옵션**:
  * **A안 (권장)**: 이미지 하단에 **폴라로이드 형태의 흰색/파스텔 프레임 스트립**을 추가하여 텍스트 명시.
  * B안: 이미지 화면 내부 좌측 하단에 반투명 글래스모피즘 뱃지로 텍스트 겹침 렌더링.
* **권장 이유**: 폴라로이드 포토 카드 스타일(A안)이 방 꾸미기 결과물을 '수집품'처럼 느끼게 해주며, 텍스트가 방 배경 아이템과 겹쳐 가독성이 떨어지는 문제를 근본적으로 방지합니다.

---

### ❓ 질문 4: 고양이 캐릭터 및 방 아이템 자산(Asset) 렌더링 형식 확정
* **내용**: 8가지 감정 표정 및 9개 방 슬롯 아이템의 시각적 그래픽 표현 방식은 무엇으로 할 것인가?
* **옵션**:
  * **A안 (권장)**: **SVG / Canvas 2D 기반 그래픽 렌더링** (Vector Illustration). 해상도 대응이 우수하고 프롬프트 기반 자산 생성이 용이함.
  * B안: Sprite Sheet 기반 PNG 애니메이션 자산 사용.
* **권장 이유**: Antigravity를 활용한 프롬프트 중심 프로토타이핑 및 브라우저 독립 실행 환경에서는 SVG/Canvas 2D 자산이 용량 부담이 없고 반응형 확대에도 선명함을 유지합니다.
